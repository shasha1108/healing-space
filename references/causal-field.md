...
### 一-A. 共享术语（全 Skill 单一权威定义）

| 术语 | 定义 | 禁止的别名 |
|------|------|-----------|
| **calm** | ∈ [0,1]，全局情绪状态变量。0=压抑峰值，1=治愈终态。所有视觉/音频参数的唯一自变量 | `peace` / `progress` / `healPhase` |
| **ts** | calm 的 smoothstep 变换：`c*c*(3-2*c)`。用于所有过渡类参数（非过渡类参数直接用裸 calm） | `t` / `smooth` / `eased` |
| **隐喻** | 情绪 → 物理现象的同构映射。必须是具体自然过程（扩散/剥落/结晶/流动），不是抽象概念 | `主题` / `概念` / `idea` |
| **三幕剧** | Act 1 美好展示(无交互)→Act 2 突然破坏(自动)→Act 3 亲手修复(交互驱动 calm 0→1)。不可跳过、不可颠倒 | `phase 1/2/3` / `stage` |
...
|------|--------|--------|---------|---------|
| CURL_STRENGTH | 15 | 3 | `15 - 12*ts` | JS: `PHYS.CURL_STRENGTH` |
...
| **Three.js 粒子** | CURL 15→3 | 粒子仍在剧烈旋转（感知 ~25% 平静） | 太慢——到 calm=0.7 才"刚开始安静" |
