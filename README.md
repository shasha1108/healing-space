...
| 🎨 | **p5.js 有机渲染** | 流场、Perlin/Simplex/Curl 噪声、curveVertex 笔触、SCREEN 混合多层叠加 |
| 🔊 | **Web Audio 程序化合成** | 432Hz/174Hz 三角波疗愈频率、五声音阶、棕/粉噪声、双耳节拍、AudioWorklet 空间混响 |
...
3. **音效设计 — 情绪温度**：432Hz 三角波温暖基底、噪声颜色匹配情绪（棕=安定/粉=柔和）——声音是情绪的质地
...
| 5 | 音效 432Hz/174Hz 三角波，gain ≤ 0.08，无 55Hz 持续低频 | 音频 |
...
```
healing-space/
├── SKILL.md                          # 完整 skill 定义（v8.0.0，硬上限 500 行）
├── README.md                         # ← 你在看这里
├── llms.txt                          # Agent-first 索引（Claude、GPT、Gemini 用）
├── CLAUDE.md                         # Git 工作流 + 多设备防冲突规范
├── LICENSE                           # MIT
│
├── assets/                           # 黄金示例文件
│   ├── golden-example.html           # 参考架构：状态机 + 音频 + 完整三幕剧
│   ├── archetype-time-clock.html     # 情绪原型：时间感 / 滴答 / 紧迫
│   └── archetype-unbound-mind.html   # 情绪原型：自由 / 释放 / 开阔
│
├── references/                       # 12 套技术武器库（生产级代码骨架）
│   ├── shader-patterns.md            # WebGL/GLSL 发光粒子渲染
│   ├── gpu-fluid.md                  # FBO ping-pong 流体模拟
│   ├── particle-physics.md           # 粒子运动方程（刚度/阻尼/旋度）
│   ├── p5-patterns.md                # p5.js 有机曲线、流场、笔触
│   ├── reaction-diffusion.md         # Gray-Scott 图灵图案（焦虑→秩序）
│   ├── physarum.md                   # 粘菌传输网络（孤独→连接）
│   ├── raymarching.md                # SDF 液态 3D、域扭曲（无网格）
│   ├── tsl-webgpu.md                 # WebGPU 计算着色器 + TSL
│   ├── audio-engine.md               # Web Audio API 程序化合成
│   ├── audio-worklet.md              # AudioWorklet + ConvolverNode 空间混响
│   ├── css-aesthetic.md              # CSS/GSAP 非 WebGL 疗愈美学
│   └── state-machine.md              # 情绪弧线设计 + 交互模式
│
└── evals/                            # 评估配置
    ├── evals.json
    └── trigger_evals.json
