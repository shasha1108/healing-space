...
必须用脚本跑结构性检查。人脑无法同时追踪所有 FBO 显存格式 × SDF 安全规则 × dt 钳制 × 光标双显修复 × API 兼容性。检查项至少覆盖：JS 语法、纯黑纯白检测、hex+string α 拼接、SDF 多球 smin、dt 钳制、光标双显。**致命项全修完才进质量自检。**
